...
#include "include/workload_vel.h"
...
void *thread_test(void *arg){
    ...
    int cid = nn_args->cid;
    bool drop_enable = nn_args->drop_enable;
    ...
    while(!global_end){ 
...
#endif
      int queue_id = get_block_workload(cid, current_time, drop_enable);
#if PRINT == 1
      ...
      while(queue_id == -1){
        ...
        queue_id = get_block_workload(cid, current_time, drop_enable);
      }
      ...
      */
      ...
      uint64_t this_runtime = workload_block_function(queue_id, cid, false); // ToDo: weight direct dram
...
int main() {
	...
	for(int i = 0; i < NUM_CORE; i++){
	  ...
	  if (t!=0) nn_args[i].drop_enable = true;
	  else nn_args[i].drop_enable = false;
	...
	for(int i = 0; i < total_workloads; i++){
	  ...
	  printf("queue id %d dropped: %d\n", i, total_queue_dropped[i]);
	  ...
	  printf("queue id %d core: %llu\n", i, total_queue_core[i]);
          printf("queue id %d throttle: %d\n", i, total_queue_throttle[i]);
